...
const mergedPRs = fs.readFileSync(path.join(__dirname, '../message/merged.md'), 'utf8');
...
export async function closed(appOctokit, prMerged, repoOwner, repoName, repoFullName, prNumber, prUsername, prSender) {
    let conn;
    try {
        if (prMerged === true) {
            ...
            console.log(`Sent a merged message to #${prNumber} on https://github.com/${repoFullName}`);
            ...
            console.log(`Removed all labels from #${prNumber} on https://github.com/${repoFullName}`);
        } else {
